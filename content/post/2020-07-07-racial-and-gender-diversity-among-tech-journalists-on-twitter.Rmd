...
# Read Write, Forbes Tech, and TechDirt were queried but excluded because
...
The total number of accounts identified in this fashion was 1740, including some false positives. So we manually went through the results and narrowed the dataset down. We then conducted the subjective assignment of race and gender. We also standardized names and removed emojis in preparation for the next stage of analysis. This process left 824 individuals. There are a few duplicates (people who have written for multiple tech journalism outlets), which we retain for our diversity analysis at the outlet-level. We remove duplicates otherwise.
...
df %>%
  ...
  labs(x="", y="Percent White",
       ...
       subtitle = "Race inferred by subjective judgment. N=824.",
...
According to our subjective judgment, 520 are likely White, 84 Asian, 15 Black, and 4 Hispanic.

Also according to our subjective judgment, 430 are Male and 287 are Female.
...
race.profiles.df$likely_race<-ifelse(race.profiles.df$probability_black>.5 &
...
According to the automated data-driven method, 398 are likely White, 44 Asian, 16 Black, and 23 Hispanic. The rest could not be classified.
...
ggplot(aes(x = race)) +  
  ...
  labs(title="Racial Diversity Among Tech Journalists on Twitter",
       subtitle="Race inferred from surname using Census data. N=481.",
      x="",
       y="Proportion",
      caption="By Justin Murphy (jmrphy.net, @jmrphy)") +
  coord_flip()
...
ggplot(aes(x = gender)) +  
  ...
  labs(title="Gender Diversity Among Tech Journalists on Twitter",
       subtitle="Gender inferred from first name using SSA data. N=772.",
      x="",
       y="Proportion",
      caption="By Justin Murphy (jmrphy.net, @jmrphy).") +
  coord_flip()
...
ggplot(aes(x = race)) +  
  ...
  labs(title="Racial and Gender Diversity Among Tech Journalists on Twitter",
       subtitle="Race and gender inferred from names using government data. N=481.",
      x="",
       y="Proportion",
      caption="By Justin Murphy (jmrphy.net, @jmrphy).") +
...
ggplot(aes(x = subjective_gender)) +  
  ...
  theme_bw() +
  labs(title="Gender Diversity Among Tech Journalists on Twitter",
       subtitle="Gender inferred by subjective judgment. N=717.",
      x="",
       y="Proportion",
      caption="By Justin Murphy (jmrphy.net, @jmrphy).") +
  coord_flip()
...
ggplot(aes(x = subjective_race)) +  
  ...
  theme_bw() +
  labs(title="Racial and Gender Diversity Among Tech Journalists on Twitter",
       subtitle="Race and gender inferred by subjective judgment. N=717.",
      x="",
       y="Proportion",
      caption="By Justin Murphy (jmrphy.net, @jmrphy).") +
...
df %>%
  ...
  labs(x="", y="Percent White",
       title="Racial Diversity Across 13 Major Tech-Journalism Outlets",
       subtitle = "Race inferred from surnames using government data. N=481.",
       caption= "By Justin Murphy (jmrphy.net, @jmrphy).")
...
df %>%
  ...
  labs(x="", y="Percent Male",
       title="Gender Diversity Across 13 Major Tech-Journalism Outlets",
       subtitle = "Gender inferred by subjective judgment. N=772.",
       caption= "By Justin Murphy (jmrphy.net, @jmrphy).")
...
The basic counts are: 165 likely to be White, 17 likely to be Asian, 23 likely to be Black, and 8 likely to be Hispanic. The rest could not be classified.
...
# To partially mitigate the problem of under-identifying black people,
# any surname with a probability of being black greater than .5 will
# will be classified as black. This will increase the error rate of
# classifying white people as black, but decrease the error rate of
# classifying black people as white.

race.lists.df$likely_race<-ifelse(race.lists.df$probability_black>.5 &
